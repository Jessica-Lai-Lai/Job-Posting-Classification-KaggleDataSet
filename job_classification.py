...
def prepare_dataset(filename):
    ...
    #########################################<Classification2.>##############################################################
    # Create Xr for fraudulent classification
    #df['has_company_logo'] = df['has_company_logo'].astype(str)

    #Xr = df['description']+ ' ' + df['has_company_logo']
    #Xr = df['description']

    #print(f"Xr: {Xr}")

    # Create target labels for fraudulent classification
    #y = df['fraudulent']

    #########################################<Classification1.>##############################################################
    ## Create Xr for education classification
    ...
    print(df['required_education'].value_counts() ) # Checking distribution of target classes

    ## Shuffle the rows
    df = df.sample(frac=1, random_state=random_state).reset_index(drop=True)

    ## Use the "description", 'title', 'company_profile' and 'requirements' column as the feature (X)
    Xr = df['description'] + ' ' + df['title'] + ' ' + df['company_profile'] + ' ' + df['requirements']

    ## Create target labels for education classification
...
def analyse_classification_tfidf(Xr_train, y_train, Xr_val, y_val, Xr_test, y_test):
    ...
    # Change to test for different search ways
    C = search_C(X_train, y_train, X_val, y_val)
    #C = search_C_Bayes(X_train, y_train, X_val, y_val)
    #C = search_C_CV(X_train, y_train, X_val, y_val)
    print(f'Best C found: {C}')
